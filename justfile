deploy:
    cargo-valence --socket https://service.coprocessor.valence.zone \
      ...
      --circuit erc20-balance-circuit | jq -r '.controller' > .controller
...
debug log_index='0':
    curl -X POST https://service.coprocessor.valence.zone/api/registry/controller/$(cat .controller)/witnesses \
